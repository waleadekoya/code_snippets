...
class JobsAPI:
    ...
    JOBSERVE_URL = "https://www.jobserve.com/gb/en/JobListingBasic.aspx?shid=70C465132748773534BA"
    ...
    def __init__(self, keyword, starting_salary=45000):
        ...
        # self.jobserve_results_queue = Queue()
        ...
        self.total_jobs_links = self.initialise_total_jobs_links()
        # self.jobserve_job_links = self.initialise_jobserve_links()

        self.total_links = len(self.reed_job_links) + len(self.indeed_job_links) \
                           + len(self.total_jobs_links)  # + len(self.jobserve_job_links)
        ...
        self.reed_job_links = self.reed_results_queue.get()
        self.indeed_job_links = self.indeed_results_queue.get()
        self.total_jobs_links = self.total_jobs_results_queue.get()
        # self.jobserve_job_links = self.jobserve_results_queue.get()

        frames = [self.reed_job_links, self.indeed_job_links, self.total_jobs_links, ]  # self.jobserve_job_links ]
        self.final_results = pd.concat(frames)
    ...
    def write_to_disk(self):
        self.final_results.to_excel(self.XLSX_FILE.format(self.keyword, datetime.utcnow().strftime(self.TIME_FORMAT)),
                                    index=False)
        self.final_results.to_csv(self.CSV_FILE.format(self.keyword, datetime.utcnow().strftime(self.TIME_FORMAT)),
    ...
    @timer
    def get_jobserve_page_count(self):
        jobserve_soup = self.make_soup(self.JOBSERVE_URL)
        results_count = int(jobserve_soup.find('span', attrs={'class': 'resultnumber'}).text)
        if results_count:
            if results_count < self.JOBSERVE_RESULTS_PER_PAGE:
                return int(results_count)
            else:
                return int(round(results_count / self.JOBSERVE_RESULTS_PER_PAGE))
    ...
    @timer
    def initialise_jobserve_links(self):
        urls = (self.JOBSERVE_DYNAMIC_URL.format(page) for page in range(1, self.get_jobserve_page_count() + 1))
    ...
    def get_indeed_jobs(self):
        ...
        self.indeed_results_queue.put(self.df)
    ...
    def get_reed_jobs(self):
        for job_link in self.reed_job_links:
            ...
            self.append_to_pandas_df(title, salary, location, job_link, advertiser, job_type, description)
        self.reed_results_queue.put(self.df)
    ...
    def get_total_jobs(self):
        for job_link in self.total_jobs_links:
            ...
            self.append_to_pandas_df(title, salary, location, job_link, advertiser, job_type, description)
        self.total_jobs_results_queue.put(self.df)
    ...
    def get_jobserve_jobs(self):
        for job_link in self.jobserve_job_links:
            ...
            self.append_to_pandas_df(title, salary, location, job_link, advertiser, job_type, description)
        self.jobserve_results_queue.put(self.df)


if __name__ == "__main__":
    JobsAPI(keyword='python', starting_salary=70000)
